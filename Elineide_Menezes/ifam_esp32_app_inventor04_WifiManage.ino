...
void setup()
{
  ...
  IPAddress ip(192, 168, 2, 162); //endereÃ§o ip para conectar Ã  pÃ¡gina web
  IPAddress gateway(192, 168, 2, 1); //Gateway padrÃ£o
  IPAddress subnet(255, 255, 255, 0); //mÃ¡scara de rede

  WiFi.config(ip, gateway, subnet);

  Serial.println("");
  Serial.println("O Wifi estÃ¡ conectado.");
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (currentLine.length() == 0) {
            ...
            client.println("<html>");
            client.println("<head>");
            client.println("<title> AutomaÃ§Ã£o Residencial </title>");
